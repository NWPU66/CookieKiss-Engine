...
// layout(local_size_x=16,local_size_y=8,local_size_z=1)in;//线程块大小，每个线程块包含16x8x1个线程
...
//     word=(word>>22)^word;
//     return float(word)/4294967295.f;
// }
...
//     if(direction.y>0.f)
//     {
//         return mix(vec3(.25f),vec3(.25f,.5f,1.f),direction.y);
//     }
//     else
//     {
//         return vec3(.03f);
...
//     vec3 barycentrics=vec3(0.,rayQueryGetIntersectionBarycentricsEXT(rayQuery,true));
...
//     const uvec2 pixel=gl_GlobalInvocationID.xy;//图像原点在左上角
//     if((pixel.x>=resolution.x)||(pixel.y>=resolution.y))
//     {
//         return;
//     }
    
//     uint rngState=resolution.x*pixel.y+pixel.x;// Initial rng seed
    
//     const vec3 cameraOrigin=vec3(-.001,1.,6.);
//     const float fovVerticalSlope=1./5.;
    
//     vec3 summedPixelColor=vec3(0.);
...
//         const vec2 screenUV=vec2((2.*randomPixelCenter.x-resolution.x)/resolution.y,//FIXME - 应该是resolution.x
//         -(2.*randomPixelCenter.y-resolution.y)/resolution.y);// Flip the y axis
        
//         // Create a ray direction:
//         vec3 rayDirection=normalize(vec3(fovVerticalSlope*screenUV.x,fovVerticalSlope*screenUV.y,-1.));
        
//         vec3 accumulatedRayColor=vec3(1.);// The amount of light that made it to the end of the current ray.
...
//                 rayOrigin=hitInfo.worldPosition+.0001*hitInfo.worldNormal;
...
//                 rayDirection=normalize(hitInfo.worldNormal+vec3(r*cos(theta),r*sin(theta),u));
//             }
//             else{
//                 // Ray hit the sky
...
//     // imageData[linearIndex]=vec3(1,0,0);
// }
